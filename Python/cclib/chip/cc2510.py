...
class CC2510(ChipDriver):
    ...
    def initialize(self):
        ...
        self.chipInfo = {"flash": 16, "usb": 0, "sram": 2}
    ...
    def writeFlashPage(self, address, inputArray, erase_page=True):
        ...
        routine8_1 = [
            ...
            0x75,
            0xAD,
            ((address >> 8) / self.flashWordSize) & 0x7E,  # MOV FADDRH, #imm;
            0x75,
            0xAC,
            0x00,  # MOV FADDRL, #00;
        ]
        routine8_erase = [
            0x75,
            0xAE,
            0x01,  # MOV FLC, #01H; // ERASE
            # ; Wait for flash erase to complete
            0xE5,
            0xAE,  # eraseWaitLoop:  MOV A, FLC;
            0x20,
            0xE7,
            0xFB,  # JB ACC_BUSY, eraseWaitLoop;
        ]
        routine8_2 = [
            # ; Initialize the data pointer
            0x90,
            0xF0,
            0x00,  # MOV DPTR, #0F000H;
            # ; Outer loops
            0x7F,
            (((words_per_flash_page) >> 8) & 0xFF),  # MOV R7, #imm;
            0x7E,
            ((words_per_flash_page) & 0xFF),  # MOV R6, #imm;
            0x75,
            0xAE,
            0x02,  # MOV FLC, #02H; // WRITE
            # ; Inner loops
            0x7D,
            self.flashWordSize,  # writeLoop:          MOV R5, #imm;
            0xE0,  # writeWordLoop:          MOVX A, @DPTR;
            0xA3,  # INC DPTR;
            0xF5,
            0xAF,  # MOV FWDATA, A;
            0xDD,
            0xFA,  # DJNZ R5, writeWordLoop;
            # ; Wait for completion
            0xE5,
            0xAE,  # writeWaitLoop:      MOV A, FLC;
            0x20,
            0xE6,
            0xFB,  # JB ACC_SWBSY, writeWaitLoop;
            0xDE,
            0xF1,  # DJNZ R6, writeLoop;
            0xDF,
            0xEF,  # DJNZ R7, writeLoop;
                ...
                for i in range(0, iLen):
                    if verifyBytes[i] != data[iOfs + i]:
                        if flashRetries < 3:
                            ...
                            raise IOError(
                                "Flash verification error on offset 0x%04x"
                                % (fAddr + i)
                            )
            flashRetries = 0
