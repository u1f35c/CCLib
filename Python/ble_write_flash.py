...
if not hasLicense:
    if opts["license"] is None:
        ...
        if opts["addr"] is None:
            if not hasLicense:
                binfo["btaddr"] = "".join(
                    ["%s:" % serial[x : x + 2] for x in range(0, len(serial), 2)]
                )[0:-1]
...
hexFile.set(
    dbg.flashSize - 57,
    [int(binfo["license"][x : x + 2], 16) for x in range(0, len(binfo["license"]), 2)],
)
hexFile.set(dbg.flashSize - 25, [binfo["hwver"]])
hexFile.set(
    dbg.flashSize - 22,
    [int(binfo["btaddr"][x : x + 2], 16) for x in range(0, len(binfo["btaddr"]), 3)],
)
